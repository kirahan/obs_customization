    ...
    ui(new Ui::FaceRec)
{
    ...
    camera->setViewfinder(viewfinder);
    camera->start(); //启动摄像头
    connect(imageCapture, SIGNAL(QCameraImageCapture::imageCaptured(int, QImage)), this, SLOT(displayImage(int, QImage)));
}
FaceRec::~FaceRec()
{
    delete ui;
...
{
	int image_int;
    image_int = imageCapture->capture();
    qDebug() << "aaa:showTheCapture" << endl;
}
void FaceRec::displayImage(int, QImage image)
{
    qDebug() << "aaa:displayImage" << endl;
